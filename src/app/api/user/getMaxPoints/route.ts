...
// Get user max point for one user to display on website
export async function GET(req: Request) {
  ...
  try {
    ...
    const userPoint = await client.query(`SELECT max_points FROM users WHERE clerk_id = $1`, [clerk_id]);
    ...
    console.error("Error fetching max points for user", error);

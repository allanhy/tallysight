...
export async function GET(req: Request) {
  ...
  try {
    ...
    await db`INSERT INTO admins (user_id, follower_count) VALUES (${user_id}, ${follower_count})  ON CONFLICT (user_id) DO NOTHING;`;
  } catch (error) {
    return NextResponse.json({ error }, { status: 500 });
  }
 
  const influencers = await db`SELECT * FROM influencers;`;
